...
- **Stress non-triviality justifies a card, but only for end-stressed `-ere` verbs** — a narrow, deliberate carve-out from the non-triviality rule above. A cognate-transparent verb (*temere, cadere, valere*) qualifies purely because its ending-stress isn't derivable from Spanish, even though its meaning is. This does **not** extend to any other word class; see marker 1 below for the closed scope.
...
**1. Stress — `-ere` verbs only, and only the end-stressed ones.** This marker has exactly one job: flagging which `-ere` verbs stress the ending. **Nothing else in the deck ever gets a capital.**

- **Scope is closed.** Only `-ere` infinitives. Not nouns, not adjectives, not `-are`/`-ire` verbs, not any other word — for those, stress transfers from the Spanish cognate (*mèdico/médico*, *diffìcile/difícil*) and the marker would be noise. `-ere` verbs are the sole exception because Spanish gives no signal at all: Spanish `-er/-ir` verbs are uniformly end-stressed.
- **A capital means end-stressed.** Mark the vowel before `-re`: *potEre, dovEre, volEre, sapEre, avEre, vedEre, tenEre, rimanEre, piacEre, cadEre, temEre, tacEre, valEre, parEre, bEre, sedErsi, godErsi*. Closed class, ~22 base verbs.
- **No capital means stem-stressed** — *prendere, scrivere, mettere, leggere, chiudere, vivere, correre…*, the much larger group. **These stay lowercase.** The absence of a mark is the information; capitalising them would destroy the contrast the marker exists to draw.
- Mark **the answer/headword only** — never inside example sentences or parenthetical notes. Derivatives (*ottenere, prevedere, mantenere*) don't get their own cards, but are marked if already present for meaning.

The deck owns this outright — dropped from `LEARNING_ROADMAP.md` 2026-07-28, so there is no notebook section and no separate stress deck; meaning and stress ride the same card.
